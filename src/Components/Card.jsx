    ...
    <>
      ...
      >
        ...
        <div className="w-full sm:w-1/4 relative">
          <img
            src={InfoData[0].img || "/placeholder.svg"}
            alt=""
            className="w-full h-[390px] object-cover rounded-md"
          />
          {/* Text Overlay (Always Visible) */}
          <div className="absolute inset-0 flex flex-col justify-center items-center text-center">
            <h2 className="text-[18px] font-bold text-[#3E2723]">VIEW COLLECTIONS</h2>
            <h1 className="text-3xl font-bold text-[#3E2723]">LOOKBOOK</h1>
            <p className="text-[#3E2723]">your world of fashion in numbers</p>
          </div>
        </div>
          ...
          {getProduct.slice(0, 4).map((product, index) => (
            ...
            >
              <div className="w-full cursor-pointer">
                ...
                />
                <h1 className="font-medium text-[18px] text-[#3E2723] mt-2">{product.name}</h1>
                <p className="text-[16px] text-[#3E2723]">${product.price}</p>
              </div>
            </Link>
          ))}
        </div>
